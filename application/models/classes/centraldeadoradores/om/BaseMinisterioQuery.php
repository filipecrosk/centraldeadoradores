 ...
 *
 * @method MinisterioQuery leftJoinUsuarioMinisterio($relationAlias = null) Adds a LEFT JOIN clause to the query using the UsuarioMinisterio relation
 * @method MinisterioQuery rightJoinUsuarioMinisterio($relationAlias = null) Adds a RIGHT JOIN clause to the query using the UsuarioMinisterio relation
 * @method MinisterioQuery innerJoinUsuarioMinisterio($relationAlias = null) Adds a INNER JOIN clause to the query using the UsuarioMinisterio relation
 *
...
abstract class BaseMinisterioQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'centraldeadoradores', $modelName = 'Ministerio', $modelAlias = null)
    {
     ...
     * @param     MinisterioQuery|Criteria $criteria Optional Criteria to build the query from
     *
     ...
     *
     * @return   Ministerio|Ministerio[]|mixed the result, formatted by the current formatter
     */
    ...
    {
        $sql = 'SELECT `ID`, `NOME` FROM `ministerio` WHERE `ID` = :p0';
        try {
        ...
        if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
            $obj = new Ministerio();
            $obj->hydrate($row);
            MinisterioPeer::addInstanceToPool($obj, (string) $key);
        }
        $stmt->closeCursor();
     ...
     * @param     PropelPDO $con an optional connection object
     *
     * @return PropelObjectCollection|Ministerio[]|mixed the list of results, formatted by the current formatter
     */
    ...
    public function filterByUsuario($usuario, $comparison = null)
    {
        if ($usuario instanceof Usuario) {
            return $this
                ->addUsingAlias(MinisterioPeer::ID, $usuario->getIdMinisterio(), $comparison);
    ...
    public function filterByUsuarioMinisterio($usuarioMinisterio, $comparison = null)
    {
        if ($usuarioMinisterio instanceof UsuarioMinisterio) {
            return $this
                ->addUsingAlias(MinisterioPeer::ID, $usuarioMinisterio->getIdMinisterio(), $comparison);
