 ...
 *
 * @method EmailHeaderQuery leftJoinArquivoEmail($relationAlias = null) Adds a LEFT JOIN clause to the query using the ArquivoEmail relation
 * @method EmailHeaderQuery rightJoinArquivoEmail($relationAlias = null) Adds a RIGHT JOIN clause to the query using the ArquivoEmail relation
 * @method EmailHeaderQuery innerJoinArquivoEmail($relationAlias = null) Adds a INNER JOIN clause to the query using the ArquivoEmail relation
 *
...
abstract class BaseEmailHeaderQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID_EMAIL`, `ID_USUARIO`, `DATA_CADASTRO`, `ASSUNTO`, `CORPO_MENSAGEM` FROM `email_header` WHERE `ID_EMAIL` = :p0';
        try {
    ...
    public function filterByArquivoEmail($arquivoEmail, $comparison = null)
    {
        if ($arquivoEmail instanceof ArquivoEmail) {
            return $this
                ->addUsingAlias(EmailHeaderPeer::ID_EMAIL, $arquivoEmail->getIdEmail(), $comparison);
